...
export interface CrumEvent {
  ...
  actor: Actor | null
  /** "<entity>.<verb>", e.g. person.create, deal.stage_change, tag.add */
  type: string
...
export function serializeEvent(ev: CrumEvent): string {
  return JSON.stringify(ev)
}

export function parseEvent(line: string): CrumEvent | null {
  const trimmed = line.trim()
  if (!trimmed) return null
  try {
    const obj = JSON.parse(trimmed) as CrumEvent
