...
import { type Actor, type CrumEvent, type Draft, serializeEvent } from './events'
...
async function resolveMe(): Promise<Actor> {
  ...
  let id = localStorage.getItem('crum.actor.id')
  if (!id) {
    id = `local_${ulid()}`
    localStorage.setItem('crum.actor.id', id)
  }
  const name = localStorage.getItem('crum.actor.name') || 'You'
  return { id, name }
}
...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    async function boot() {
      ...
      // very first event) to appear. We don't pre-subscribe a not-yet-created
      // shard: dispatch subscribes it right after it writes the file, and
      // watchFiles catches shards created remotely.
      ...
      // live" demo (mode 'demo') and in the standalone deployed demo (__crumDemo).
      if (window.gt.mode === 'demo' || window.__crumDemo) {
  ...
  async function dispatchImpl(drafts: Draft | Draft[]): Promise<void> {
    ...
    const events: CrumEvent[] = list.map((d) => ({
      id: newId('evt'),
      ts: now,
      actor: meRef.current,
      type: d.type,
