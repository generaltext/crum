...
import type { CrumEvent } from './events'

const DB_NAME = 'crum'
const STORE = 'projection'
// v3 dropped the persisted `consumed` cursor: a saved char offset is not a
...
// memory and is revalidated each session. Bumping the version discards any
// older cache that may hold state built by the old, offset-skipping fold.
...
export async function loadCache(workspaceId: string): Promise<{ state: State } | null> {
  try {
    ...
    s.comments = cached.state.comments
    s.dealStages = cached.state.dealStages
